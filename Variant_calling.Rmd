...
633 227 1 16,22 1 27,99 1 18,67  T C
654 173 1 9,18 1 3,43 1 6,32  C T
675 67 0 . 1 14,31 1 9,35  A G
681 225 1 0,32 1 0,57 1 0,48  G A
...
```{bash, eval = F}
[sgamino@compute-00-10 data]$ for strain in SRR445715 SRR445716 SRR445717; do bcftools stats SRR445715.vcf.gz | grep TSTV | cut -f5; done
...
```{bash, eval = F}
[sgamino@compute-00-10 data]$ bcftools filter -i 'QUAL>=30 && FORMAT/AD[*:1]>=50' -o combined_filtered.vcf combined.vcf
[sgamino@compute-00-10 data]$ bcftools stats combined_filtered.vcf | grep TSTV | cut -f5
# TSTV, transitions/transversions:
[5]ts/tv
3.23
...
When we put the filters, the ts/tv increases.
...
```{bash, eval = F}
[sgamino@compute-00-10 data]$ bcftools filter -sLowQual -m+ -i'QUAL>=30 && AD[*:1]>=50' -g8 -G10 combined.vcf -o combined.flt.vcf
[sgamino@compute-00-10 data]$ bcftools stats combined.flt.vcf | grep TSTV | cut -f5
# TSTV, transitions/transversions:
[5]ts/tv
2.91
```

The tsv of the removed sites is very similar to the tsv of the original combined vcf, which suggests that the quality is poor.

### Question 13: Can you see why position chrI:1101 passed these filters?

```{bash, eval = F}
[sgamino@compute-00-10 data]$ grep -w "chrI" combined.flt.vcf | grep -w "1101"
...
#### And why did position chrI:1107 fail the filtering step? 

```{bash, eval = F}
[sgamino@compute-00-10 data]$ grep -w "chrI" combined.flt.vcf | grep -w "1107"
...
### Question 14: There are three BAM files in the original directory /mnt/atgc-d2/bioinfoII/drobles/variant_calling/data/. Can you modify the command from section 3 to use all three BAM files and only write out variant sites in chromosome I? Write the output to a compressed BCF file called multi.bcf and index the file afterwards.

```{bash, eval = F}
samtools merge -@ 4 /mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/multi.bam 
/mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/SRR445715.aligned.sorted.bam 
/mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/SRR445716.aligned.sorted.bam 
/mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/SRR445717.aligned.sorted.bam


bcftools mpileup -a AD -f /mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/S288C_ref.fa \
-Ou /mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/multi.bam | \
bcftools call -mv -Ob -o /mnt/atgc-d1/bioinfoII/sgamino/variant_calling/data/multi.bcf

# The commands above are the ones i used in a job to get my multi-variant calling bcf. 

[sgamino@compute-00-04 data]$ bcftools index multi.bcf


[sgamino@compute-00-04 data]$ bcftools view -r chrI -i 'GT!="./."' -O b -o multi_chrI.bcf multi.bcf # Here, i filtered my multi.bcf for just the variants in chrI. -r restricted the filter for just the chrI, and then -i 'GT!=./.' filters for samples in which the genotype wasn't empty.

[sgamino@compute-00-04 data]$ bcftools index multi_chrI.bcf # Finally, i indexed the chrI filtered bcf.

```


### Question 15: Can you apply the same filters as before? 

```{bash, eval = F}
bcftools query -f'%POS QUAL = %QUAL [GT = %GT AD = %AD ] REF = %REF %ALT\n' -i'QUAL>=30 && AD[\*:1]\>50' multi_chrI.bcf | head
109 QUAL = 66 GT = 0/1 AD = 89,71  REF = G C
115 QUAL = 86 GT = 0/1 AD = 74,80  REF = C G
136 QUAL = 170 GT = 1/1 AD = 0,128  REF = G A
137 QUAL = 90 GT = 0/1 AD = 90,55  REF = C CT
286 QUAL = 225 GT = 1/1 AD = 0,166  REF = A T
305 QUAL = 225 GT = 1/1 AD = 0,151  REF = C G
349 QUAL = 139 GT = 0/1 AD = 130,96  REF = C T
472 QUAL = 51 GT = 0/1 AD = 81,53  REF = C T
476 QUAL = 192 GT = 1/2 AD = 0,89,37  REF = G T,C
509 QUAL = 185 GT = 0/1 AD = 44,67  REF = G A

```


#### How many sites pass the filters? Write the output to a BCF file called multi.filt.bcf and index the file.

```{bash, eval = F}
[sgamino@compute-00-12 data]$ bcftools view -i 'QUAL>=30 && AD[*:1]>50' -O b -o multi.filt.bcf multi_chrI.bcf
[sgamino@compute-00-12 data]$ bcftools index multi.filt.bcf
[sgamino@compute-00-12 data]$ bcftools view multi.filt.bcf | wc -l
686
```

686 sites pass the filters

### Question 16: What is the ts/tv of the raw calls and of the filtered set?

Raw calls:
```{bash, eval = F}
[sgamino@compute-00-12 data]$ bcftools stats multi_chrI.bcf | grep TSTV | cut -f5
# TSTV, transitions/transversions:
[5]ts/tv
2.62
```
...
Filtered set:
```{bash, eval = F}
[sgamino@compute-00-12 data]$ bcftools stats multi.filt.bcf | grep TSTV | cut -f5
# TSTV, transitions/transversions:
[5]ts/tv
2.94
```
...
> I think a good way to obtain a list of the candidates would be to check the sites that are involved in the pathway of substrate consumption. We could also see the mutations that are not shared in the mutated strain.

Hint: You can assume that variants shared by both IMW004 and IMW005 are likely to
...
#### How many variants (unfiltered) are in IMW004 that are not shared by any other strain?

```{bash, eval = F}
[sgamino@compute-00-12 data]$ bcftools isec -C SRR445716.vcf.gz SRR445715.vcf.gz SRR445717.vcf.gz > present_in_IM004_absent_in_CEN.PK113-7D_and_IM005.txt
Note: -w option not given, printing list of sites...
[sgamino@compute-00-12 data]$ wc -l present_in_IM004_absent_in_CEN.PK113-7D_and_IM005.txt
209 present_in_IM004_absent_in_CEN.PK113-7D_and_IM005.txt
```

> There are 209 sites in the IMW004 strain that are not present in the others.

#### Now, can you apply filters to remove those sites that are not
well-covered? let’s set it at DP\>30 and QUAL\>50.

```{bash, eval = F}
[sgamino@compute-00-12 data]$ bcftools isec -O z -o present_in_IM004_absent_in_CEN.PK113-7D_and_IW005.vcf.gz -C -w 1 SRR445716.vcf.gz SRR445715.vcf.gz SRR445717.vcf.gz
[sgamino@compute-00-12 data]$ bcftools filter -i 'QUAL >= 50 && DP >= 30' -o present_in_IM004_absent_in_CEN.PK113-7D_and_IW005_FILTERED.vcf.gz present_in_IM004_absent_in_CEN.PK113-7D_and_IW005.vcf.gz 
```
...
```{bash, eval = F}
[sgamino@compute-00-12 data]$ bcftools view -H present_in_IM004_absent_in_CEN.PK113-7D_and_IW005_FILTERED.vcf.gz | wc -l
21
```
...
```{bash eval = F}
[sgamino@compute-00-12 data]$ bcftools isec -O z -o present_in_IMW005_absent_in_CEN.PK113-7d_and_IMW005.vcf.gz -w 1 -C SRR445717.vcf.gz SRR445716.vcf.gz SRR445715.vcf.gz
[sgamino@compute-00-12 data]$ mv present_in_IMW005_absent_in_CEN.PK113-7d_and_IMW005.vcf.gz present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004.vcf.gz
[sgamino@compute-00-12 data]$ bcftools filter -i 'QUAL >= 50 $$ DP >=30' -o present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004_FILTERED.vcf.gz present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004.vcf.gz
[filter.c:2491 filters_init1] Error: the tag "$$" is not defined in the VCF header
[sgamino@compute-00-12 data]$ bcftools filter -i 'QUAL >= 50 && DP >=30' -o present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004_FILTERED.vcf.gz present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004.vcf.gz
```
...
```{bash eval = F}
[sgamino@compute-00-12 data]$ bcftools view -H present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004_FILTERED.vcf.gz | wc -l
16
```


## 2. Running Ensembl VEP and obtaining the information about biological
consequences
...
gunzip present_in_IM004_absent_in_CEN.PK113-7D_and_IW005_FILTERED.vcf.gz
gunzip present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004_FILTERED.vcf.gz
vep --cache --dir_cache /home/drobles/.vep/ -i -present_in_IM004_absent_in_CEN.PK113-7D_and_IW005_FILTERED.vcf.gz -o SRR445716_unique.flt.vep.vcf --vcf --species "saccharomyces_cerevisiae"
vep --cache --dir_cache /home/drobles/.vep/ -i present_in_IMW005_absent_in_CEN.PK113-7D_and_IMW004_FILTERED.vcf.gz -o SRR445717_unique.flt.vep.vcf --vcf --species "saccharomyces_cerevisiae"
```
...
- [--cache]: Enables the use of cache
- [--dir_cache]: It specifies the cache directory
- [-i]: It specifies the input file
- [-o]: It specifies the output file
- [--vcf]: It specifies to write output in vcf format.
- [--species]: It specifies the species of the variants.

### Question 21: Look at the output VCF. What happened to the original VCF?

> It now contains much more information, regarding of the variant consequence predictor analysis. The vcf function has adden information to each variant.

#### Did VEP add an annotation? Which one? 

> Yes, it added the annotation CSQ, with the format: Allele|Consequence|IMPACT|SYMBOL|Gene|Feature_type|Feature|BIOTYPE|EXON|INTRON|HGVSc|HGVSp|cDNA_position|CDS_position|Protein_position|Amino_acids|Codons|Existing_variation|DISTANCE|STRAND|FLAGS|SYMBOL_SOURCE|HGNC_ID
...
```         
Genes that are mutated in any or both of the files
...
IMW004_mutations = matrix(NA, ncol = 4, nrow = 0)
IMW005_mutations = matrix(NA, ncol = 4, nrow = 0)

IMW004_vcf = readVcf("Variant_Calling/SRR445716_unique.flt.vep.vcf", genome = "saccharomyces_cerevisae")
IMW004_csq = info(IMW004_vcf)$CSQ
IMW004_csq = as.list(IMW004_csq)

IMW005_vcf = readVcf("Variant_Calling/SRR445717_unique.flt.vep.vcf", genome = "saccharomyces_cerevisae")
IMW005_csq = info(IMW005_vcf)$CSQ
IMW005_csq = as.list(IMW005_csq)

for (var in IMW004_csq) {
  for (i in 1:length(var)) {
    var_split = strsplit(var, "\\|")
    IMW004_mutations = rbind(IMW004_mutations, c(var_split[[i]][2], var_split[[i]][3], var_split[[i]][4], var_split[[i]][5]))
  }
}
IMW004_mutations_df <- as.data.frame(IMW004_mutations)
#Aquí ya tenemos un data frame con los porcentajes de identidad y ids.

for (var in IMW005_csq) {
  for (i in 1:length(var)) {
    var_split = strsplit(var, "\\|")
    IMW005_mutations = rbind(IMW005_mutations, c(var_split[[i]][2], var_split[[i]][3], var_split[[i]][4], var_split[[i]][5]))
  }
}
IMW005_mutations_df <- as.data.frame(IMW005_mutations)

#Here we can know the mutation shared in borth strains
IMW004_IMW005_mutations <- merge(IMW004_mutations, IMW005_mutations)
print(IMW004_IMW005_mutations)
```
...
> Yes, as you can see in the code above, one of the shared mutations in both IMW004 and IMW005 strains is indeed the one in ADY2, a missense variant.
...
```{r}
IMW004_IMW005_mutations_filtered = matrix(NA, ncol = 4, nrow = 0)

for (var in 1:nrow(IMW004_mutations_df)){
  mutation_type <- IMW004_mutations[var, 1]
  if (mutation_type == "frameshift_variant" | mutation_type == "missense_variant" | mutation_type == "stop_gained" | mutation_type == "splice_acceptor_variant" | mutation_type == "splice_donor_variant"){
    IMW004_IMW005_mutations_filtered <- rbind(IMW004_IMW005_mutations_filtered, IMW004_mutations[var,])
  }
}
for (var in 1:nrow(IMW005_mutations_df)){
  mutation_type <- IMW005_mutations[var, 1]
  if (mutation_type == "frameshift_variant" | mutation_type == "missense_variant" | mutation_type == "stop_gained" | mutation_type == "splice_acceptor_variant" | mutation_type == "splice_donor_variant"){
    IMW004_IMW005_mutations_filtered <- rbind(IMW004_IMW005_mutations_filtered, IMW005_mutations[var,])
  }
}
IMW004_IMW005_mutations_filtered <- unique(IMW004_IMW005_mutations_filtered)
(IMW004_IMW005_mutations_filtered_df = as.data.frame(IMW004_IMW005_mutations_filtered))
```
...
# We will use the lookup extensión to gather as many data as we can from our candidate genes.
lookup_ext_post = "lookup/id/"
ids_query = toJSON(list(ids=IMW004_IMW005_mutations_filtered_df$V4))

lookup_res = fetch_endpoint_POST(ensembl_server, lookup_ext_post, ids_query, "application/json")

IMW004_IMW005_mutations_filtered_info = c()

for (gene in lookup_res) {
  IMW004_IMW005_mutations_filtered_info = c(IMW004_IMW005_mutations_filtered_info, paste0(gene$object_type, "{", gene$display_name, "{", gene$db_type, "{", gene$source, "{", gene$canonical_transcript, "{", gene$logic_name, "{", gene$biotype, "{", gene$description, "{", gene$assembly_name, "{", gene$end, "{", gene$start, "{", gene$seq_region_name, "{", gene$strand, "{", gene$id))  
}

IMW004_IMW005_mutations_filtered_info = as.data.frame(IMW004_IMW005_mutations_filtered_info)
# Aquí está contenida toda la información acerca de los genes presentes en ambas cepas
IMW004_IMW005_mutations_filtered_info_df = separate(IMW004_IMW005_mutations_filtered_info, IMW004_IMW005_mutations_filtered_info, into = c("Object type", "Display name", "db type", "Source", "Canoical transcript", "Logic name", "Biotype", "Description", "Assembly name", "End", "Start", "Seq region name", "Strand", "Gene id"), sep = "{")
